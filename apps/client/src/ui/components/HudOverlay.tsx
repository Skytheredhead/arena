...
/* Individual feed entry — slides in, fades after its display window */
function FeedItem({ entry, index, nowMs }: { entry: KillFeedEntry; index: number; nowMs: number }): React.JSX.Element | null {
  const age = nowMs - entry.tick;
  const fadeStart = 8_500;
  const fadeEnd   = 10_000;
  const opacity = age < fadeStart ? 1 : age > fadeEnd ? 0 : 1 - (age - fadeStart) / (fadeEnd - fadeStart);
  ...
  return (
    <div
      style={{
        ...
        fontFamily: CYBER.font, fontSize: '10px', lineHeight: 1.3,
        opacity,
    ...
    >
      <span style={{ color: isKill ? CYBER.a2 : CYBER.a, fontWeight: 700, textShadow: `0 0 6px ${isKill ? CYBER.a2 : CYBER.a}88` }}>
...
export function HudOverlay({
  ...
  // Trigger ammo number tick animation on change
  useEffect(() => {
  ...
  // Low HP: pulsing border on the panel
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-20">
      ...
      {damageFlashToken > 0 && (
        <div
          ...
          style={{
            ...
            background: 'radial-gradient(ellipse at center,rgba(255,32,32,0.02) 28%,rgba(255,32,32,0.18) 62%,rgba(100,0,0,0.55) 100%)',
            animation: 'cyberDamageFlash .55s cubic-bezier(.16,1,.3,1) both',
          }}
        />
      )}

      <CyberCrosshair hitmarkerVisible={hitmarkerVisible} spread={crosshairSpread} scoped={scoped} />
      ...
      >
        <CyberPanel style={{
          ...
          <div style={{ textAlign: 'center', width: '72px' }}>
            <div style={{
              fontFamily: "'Orbitron',var(--font)", color: CYBER.textBright,
            ...
            <div style={{ color: CYBER.textBright, fontSize: '8px', letterSpacing: '3px', fontFamily: CYBER.font }}>KILLS</div>
          </div>
          <div style={{ width: '1px', height: '28px', background: CYBER.border }} />
          <div style={{ textAlign: 'center', width: '72px' }}>
            <div style={{
              fontFamily: "'Orbitron',var(--font)", color: CYBER.textBright,
              fontSize: '20px', fontWeight: 700,
            ...
            <div style={{ color: CYBER.textBright, fontSize: '8px', letterSpacing: '3px', fontFamily: CYBER.font }}>DEATHS</div>
          </div>
        </CyberPanel>
      </div>

      {/* ── TOP RIGHT: chat / kill feed ── */}
      <div
        className="cyber-slide-right"
        style={{
          position: 'absolute', top: '14px', right: '14px', zIndex: 10,
          width: 'min(340px,45vw)', pointerEvents: 'auto',
        }}
      >
        <CyberPanel style={{ padding: '8px', background: 'rgba(0,10,20,0.85)', backdropFilter: 'blur(10px)' }}>
          <div style={{
            color: CYBER.textBright, fontFamily: CYBER.font, fontSize: '9px',
          ...
          <form
            onSubmit={e => { e.preventDefault(); onChatSend(); }}
          ...
          >
            <input
              ...
              placeholder={chatOpen ? 'Type and press Enter…' : 'Press / to chat'}
      ...
      <div
        className="cyber-slide-right"
        style={{ position: 'absolute', bottom: '80px', right: '24px', zIndex: 10, textAlign: 'right' }}
      >
        <CyberPanel style={{ padding: '16px 20px', backdropFilter: 'blur(12px)' }}>
          <div style={{
            color: CYBER.textBright, fontSize: '9px', letterSpacing: '3px',
            fontFamily: CYBER.font, marginBottom: '6px', textAlign: 'right',
          }}>
            PULSE RIFLE
          </div>
          ...
          <div style={{ display: 'flex', gap: '2px', justifyContent: 'flex-end', marginTop: '8px', flexWrap: 'wrap', maxWidth: '120px', marginLeft: 'auto' }}>
            {Array.from({ length: RIFLE_MAGAZINE }, (_, i) => (
              <div
                key={i}
                style={{
      ...
      <div
        ...
        style={{
          ...
          background: 'rgba(2,11,20,0.88)', backdropFilter: 'blur(8px)',
      ...
      {scoreboardOpen && (
        <div style={{
          ...
          zIndex: 25,
          background: 'rgba(2,11,20,0.8)', backdropFilter: 'blur(6px)',
        }}>
          <div className="cyber-scale-in" style={{ width: 'min(1080px,96vw)', maxHeight: '86vh', overflow: 'hidden' }}>
            <CyberPanel style={{ overflow: 'hidden', boxShadow: `0 0 40px ${CYBER.a}18` }}>
              {/* Scoreboard header */}
              <div style={{
                background: 'rgba(2,11,20,0.97)', backdropFilter: 'blur(12px)',
                borderBottom: `1px solid ${CYBER.border}`,
                padding: '16px 28px',
                display: 'flex', justifyContent: 'space-between', alignItems: 'center',
              }}>
                <div style={{
                  fontFamily: "'Orbitron',var(--font)", color: CYBER.a,
                ...
                <div style={{ display: 'flex', gap: '36px', alignItems: 'center' }}>
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ color: CYBER.ok, fontSize: '28px', fontFamily: "'Orbitron',var(--font)", fontWeight: 700, textShadow: `0 0 10px ${CYBER.ok}88` }}>{localKills}</div>
                  ...
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ color: CYBER.danger, fontSize: '28px', fontFamily: "'Orbitron',var(--font)", fontWeight: 700, textShadow: `0 0 10px ${CYBER.danger}66` }}>{localDeaths}</div>
              ...
              <div style={{ display: 'flex', borderBottom: `1px solid ${CYBER.a}44`, background: 'rgba(0,245,255,0.03)' }}>
                {[
                  ...
                  <div
                    key={col.label}
                    style={{
                      ...(col.flex ? { flex: 1, paddingLeft: '8px' } : { width: `${col.w}px`, textAlign: 'center' }),
                      padding: '10px 8px',
                      color: col.color ?? CYBER.textDim,
                      fontSize: '9px', letterSpacing: '3px', fontFamily: CYBER.font, textTransform: 'uppercase',
                    }}
                  >
                    {col.label}
                  </div>
                ...
                {scoreboard.map((player, i) => (
                  <div
                    ...
                    style={{
                      ...
                      boxShadow: i === 0 ? `inset 3px 0 0 ${CYBER.a}` : undefined,
                      animation: `cyberSlideLeft .3s ${i * 0.04}s cubic-bezier(.16,1,.3,1) both`,
                    }}
                  >
                    <div style={{ width: '40px', padding: '13px 8px', color: CYBER.textDim, fontSize: '12px', textAlign: 'center', fontFamily: CYBER.font }}>{i + 1}</div>
                    <div style={{ flex: 1, padding: '13px 8px', color: i === 0 ? CYBER.textBright : CYBER.text, fontWeight: i === 0 ? 'bold' : 'normal', fontSize: '13px', letterSpacing: '1px', fontFamily: CYBER.font }}>
                      {player.nickname}
                      {player.identity === localIdentity && (
                        <span style={{ color: CYBER.a, fontSize: '9px', marginLeft: '8px', letterSpacing: '2px', animation: 'cyberPulse 2s infinite' }}>◀ YOU</span>
                      )}
                    </div>
                    <div style={{ width: '64px', textAlign: 'center', padding: '13px 8px', fontFamily: "'Orbitron',var(--font)", color: CYBER.ok, fontWeight: 700, fontSize: '15px', textShadow: `0 0 8px ${CYBER.ok}66` }}>{player.kills}</div>
                    ...
                    <div style={{ width: '88px', textAlign: 'center', padding: '13px 8px', fontFamily: "'Orbitron',var(--font)", color: CYBER.a, fontWeight: 700, fontSize: '15px', textShadow: `0 0 8px ${CYBER.a}88` }}>{player.kdr.toFixed(2)}</div>
